#!/usr/bin/env python3
"""
Deploy with WORKING image - installing PyTorch via pip
"""
...
# Use Ubuntu + install PyTorch manually (guaranteed to work)
...
echo "Data ready: $(wc -l < /workspace/train.jsonl) examples"
...
def deploy():
    print("=" * 70)
    print("DEPLOYMENT WITH WORKING IMAGE")
    print("=" * 70)
    print(f"\nImage: {BASE_IMAGE}")
    ...
    # Stop old pods first
    print("\n🛑 Stopping old pods...")
    old_pods = ['dbj6fou7fjjix2', 'u9w634wsvtxbmd', 'oxt1ekngidltqr']
    for pod_id in old_pods:
        try:
            requests.post(
                RUNPOD_API_URL,
                headers={'Authorization': f'Bearer {RUNPOD_API_KEY}'},
                json={'query': 'mutation { podTerminate(input: {podId: "' + pod_id + '"}) { id } }'},
                timeout=30
            )
    ...
    with open('runpod_fixed.json', 'w') as f:
        json.dump({'deployed_at': datetime.now().isoformat(), 'pods': pods, 'image': BASE_IMAGE}, f, indent=2)
    
    print(f"\n{'='*70}")
    print(f"✅ {len(pods)} pods deployed")
    return pods
