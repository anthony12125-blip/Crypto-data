#!/usr/bin/env python3
"""
RunPod Training Deployment - FIXED Schema
3 Agent Ensemble with correct GraphQL format
"""
...
def make_docker_command(agent_key, training_file):
    """Create the docker command - using string concatenation to avoid f-string issues"""
...
def deploy():
    """Deploy all 3 agents to RunPod with correct schema"""
    print("=" * 70)
    print("IRON DRAGOON - RUNPOD DEPLOYMENT (FIXED)")
    print("=" * 70)
    ...
    # Write training script to repo so pods can download it
    train_script = make_training_script()
    with open('train_pod.py', 'w') as f:
        f.write(train_script)
    print("\n✓ Created train_pod.py")
    ...
    # Save
    with open('runpod_deployed.json', 'w') as f:
